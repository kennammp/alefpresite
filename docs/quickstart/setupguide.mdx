...
If you are familiar with the process, and want to provision all elements of mobile network with a single request, jump ahead to the [**Full mobile network provisioning examples**](#full-mobile-network-provisioning-examples).
...
```
  ...
  ```bash
  curl -X 'POST' \
  'https://virtserver.swaggerhub.com/MMCKENN72_1/alef-mobile_network_api/1.0.0-oas3-mm1/connectivity/v1/mobile-network/bla/' \
  -H 'accept: */*' \
...
The following configuration steps can be done at the same time you create your Mobile Gateway (see [**Full mobile network provisioning examples**](#full-mobile-network-provisioning-examples)) or after you have created it using the `PUT` examples below.
...
<Tabs>
  ...
  ```bash
  curl -X 'PUT' \
  'https://api.alefedge.com/connectivity/v1/mobile-network/?authorization=<API_KEY>' \
  -H 'accept: */*' \
  -H 'Content-Type: application/json' \
  -d '{
  "_id": "string",
...
## Full mobile network provisioning examples

<details>
  <summary>Expand here to see all the above steps in one <code>POST</code> request. </summary>
